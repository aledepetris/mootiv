...
export class TrainingTypesService {
    ...
    getTrainingTypesByIds(ids: number[]): Observable<any[]> {
        return this.http.get<any[]>(`${this.baseUrl + this.basePath}`).pipe(
            map((types) => types.filter((type) => ids.includes(type.id)))
        );
    }
}
